...
export const test = base.extend({
    pageUser: async ({ page, context }, use) => {
        await context.storageState({ path: './test-data/states/userOneState.json' });
        await use(page);
    },
    
    garagePageAsUser: async ({ pageUser }, use) => {
        await pageUser.goto('/');
        let garagePage = new GaragePage(pageUser);
    ...
    verifyInstructionsButtonAsGuest: async ({ addAndEditCarAsGuest }, use) => {
        const instructionsButton = addAndEditCarAsGuest.page.locator('a.btn.btn-white.btn-sidebar.sidebar_btn', { hasText: 'Instructions' });
        await expect(instructionsButton).toBeVisible();
